...
for corpus in corpora:
    ...
    wavscp = open(base_dir+'/' +corpus + 'wav.scp2','w')
    fout = open(base_dir+'/' +corpus + 'text2','w')
    utt2spk = open(base_dir + '/' +corpus + 'utt2spk2', 'w')
    segment = open(base_dir + '/' +corpus + 'segments', 'w') #utt_id file_id start_time end_time
    for audio in corpus_dir.iterdir():
        ...
        wavscp.writelines(f"{corpus+'_'+ audio.name}\t{os.path.join(str(audio), audio.name)}.wav\n")
